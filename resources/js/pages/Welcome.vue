...
const props = withDefaults(
    ...
    }>(),
    {
        ...
        cases: () => [
            ...
            {
                name: 'Nuon/Vattenfall',
                slug: 'nuon-vattenfall',
                video: null,
                photo: null,
            },
        ...
        testimonialAuthorAvatar: () =>
            '/figma-assets/8a2fda0a-bf49-4b4c-b9a8-e66c433eed09.png',
        testimonialLogo: () =>
...
const SCROLL_INDICATOR_URL =
    '/figma-assets/6eea3ed9-667d-4b4e-8eb4-acc6dce262db.svg';
...
// Fallback-assets voor het testimonial-blok: tonen het Figma-ontwerp zolang het CMS leeg is.
const FALLBACK_TESTIMONIAL_LOGO =
    '/figma-assets/e6fdd583-cf9b-4016-89eb-8a00b937a435.png';
const FALLBACK_TESTIMONIAL_AVATAR =
    '/figma-assets/8a2fda0a-bf49-4b4c-b9a8-e66c433eed09.png';
...
const windowWidth = ref(
    typeof window !== 'undefined' ? window.innerWidth : FIGMA_DESKTOP_WIDTH,
);
const windowHeight = ref(
    typeof window !== 'undefined' ? window.innerHeight : FIGMA_DESKTOP_HEIGHT,
);
...
function onScroll() {
    ...
    if (testimonialRef.value)
        updateProgress(testimonialRef.value, testimonialProgress);
...
const HERO_VIDEO_EXPAND_END = 0.1;
...
const navOnYellow = computed(() => {
    if (servicesProgress.value > 0.02 && servicesProgress.value < 0.99)
        return false;
    if (casesProgress.value > 0.02 && casesProgress.value < 0.99) return false;
    if (testimonialProgress.value > 0.02 && testimonialProgress.value < 0.99)
        return false;
...
const heroVideoStyle = computed(() => {
    if (servicesProgress.value >= 1 && casesProgressRaw.value >= 0)
        return { display: 'none' as const };
    const e = 1 - ease(rp(p.value, 0, 0.1));
...
const titleStyle = computed(() => {
    const phase1 = ease(rp(p.value, 0, 0.1)); // title shrinks as video expands
    const phase2 = ease(rp(p.value, 0.1, 0.2)); // title snel uit beeld
    return {
        ...
        opacity: 1 - ease(rp(p.value, 0.13, 0.2)),
    };
});
...
const bodyTextScaleT = computed(() =>
    ease(rp(p.value, HERO_TEXT_SCALE_START, HERO_TEXT_SCALE_END)),
);
...
const bodyTextTop = computed(() => {
    const baseTop =
        (HERO_TEXT_START_TOP / FIGMA_DESKTOP_HEIGHT) * windowHeight.value;
    const t = rp(p.value, 0.085, 0.99);
...
const bodyTextContainerStyle = computed<CSSProperties>(() => {
    const gradient =
        'linear-gradient(180deg, transparent 0%, black 30%, black 55%, transparent 95%)';
    return {
        position: 'fixed',
        top: 0,
        left: 0,
...
const heroVideoOverlayOpacity = computed(() => ease(rp(p.value, 0.1, 0.35)));
...
const servicesLeadProgress = computed(() => {
    ...
    const aboutBottom = aboutEl
        ? aboutEl.offsetTop + aboutEl.offsetHeight
        : sectionTop;
...
function withScrollLocks(
    ...
    for (const point of points) {
        ...
        if (safeProgress < start) {
            return Math.max(
                0,
                Math.min((moved + (safeProgress - cursor)) * moveScale, 1),
            );
        }
...
const servicesVisualProgress = computed(() =>
    withScrollLocks(
        ...
        SERVICE_SCROLL_LOCK_POINTS,
        SERVICE_SCROLL_LOCK_SPAN,
    ),
);
...
watch(
    [
        windowScrollY,
        servicesLeadProgress,
        servicesProgress,
        servicesVisualProgress,
    ],
    ([scrollY, lp, sp, svp]) => {
...
const servicesVideoStyle = computed(() => {
    ...
    if (sp >= 1 && casesProgressRaw.value >= 0)
        return { display: 'none' as const };
    ...
    const textBottomViewport = aboutTextRef.value
        ? aboutTextRef.value.offsetTop +
          aboutTextRef.value.offsetHeight -
          windowScrollY.value
        : 194 + 297;
    ...
    const startHeight = Math.min(
        (860.054 / FIGMA_DESKTOP_HEIGHT) * windowHeight.value,
        windowHeight.value * 0.58,
    );
    const startTop = windowHeight.value + 32;
    const settledTop = Math.max(
        targetTop,
        windowHeight.value - startHeight - 24,
    );
...
const servicesScrollIndicatorOpacity = computed(() =>
    Math.min(
...
const CARD_SIZES = [
    ...
    { width: 448, height: 386 },
    { width: 451, height: 304 },
] as const;
...
const casesTrackOffset = computed(() => {
    ...
    const totalWidth = items.reduce(
        (sum, _, i) => sum + CARD_SIZES[i % CARD_SIZES.length].width,
        0,
    );
...
const TESTIMONIAL_LOGO_GAP = 80;
const TESTIMONIAL_LOGO_SPEED = 36;
const testimonialLogoOffset = ref(0);

const TESTIMONIAL_LOGO_WIDTH = 200;
const TESTIMONIAL_LOGO_HEIGHT = 100;

const testimonialLogoRowHeight = computed(() => TESTIMONIAL_LOGO_HEIGHT + 12);

// Val terug op de Figma-assets zolang het CMS nog geen content heeft.
const resolvedTestimonialLogo = computed(
    () => props.testimonialLogo || FALLBACK_TESTIMONIAL_LOGO,
);
const resolvedTestimonialAvatar = computed(
    () => props.testimonialAuthorAvatar || FALLBACK_TESTIMONIAL_AVATAR,
);
const resolvedClientLogos = computed(() =>
...
const testimonialLogoItems = computed(() => {
    const logos = resolvedClientLogos.value;
...
const testimonialLogoTrackWidth = computed(() => {
    ...
    return (
        logos.length * TESTIMONIAL_LOGO_WIDTH +
        Math.max(logos.length - 1, 0) * TESTIMONIAL_LOGO_GAP
    );
});
...
function carouselTick(timestamp: number): void {
    ...
    if (total > 0) {
        if (!carouselDragging.value && hoveredCaseIndex.value === null) {
            carouselOffset.value =
                (carouselOffset.value + CAROUSEL_SPEED * dt) % total;
        } else if (!carouselDragging.value && Math.abs(carouselVelocity) > 1) {
            carouselVelocity *= 0.9;
            carouselOffset.value =
                (((carouselOffset.value + carouselVelocity * dt) % total) +
                    total) %
                total;
        }
    }
    ...
    if (logoTotal > 0) {
        testimonialLogoOffset.value =
            (testimonialLogoOffset.value + TESTIMONIAL_LOGO_SPEED * dt) %
            logoTotal;
    }
...
function onCarouselPointerMove(e: PointerEvent): void {
    ...
    const total = casesTrackOffset.value;
    carouselOffset.value =
        (((carouselDragStartOffset + delta) % total) + total) % total;
...
const testimonialEntryProgress = computed(() => {
    ...
    return rp(
        windowHeight.value * 0.98 - rect.top,
        0,
        windowHeight.value * 0.45,
    );
});
...
function shiftTestimonialLogos(direction: 'prev' | 'next') {
    ...
    const delta = TESTIMONIAL_LOGO_WIDTH + TESTIMONIAL_LOGO_GAP;
    const nextOffset =
        direction === 'next'
...
// Exacte posities uit Figma (frame "Team Lemon", 1640×900): left/width = % van breedte, top/height = vh.
const TEAM_PHOTO_POSITIONS = [
    ...
    { left: '68%', top: '3vh', width: '13.6%', height: '32.9vh', rotate: 1.87 },
    {
        left: '92.3%',
        top: '34.8vh',
        width: '9.4%',
...
<template>
    ...
    <div :style="heroVideoStyle">
        ...
        <div
            v-else
            class="flex h-full w-full items-center justify-center bg-neutral-800"
        >
    ...
    <!-- Donkere verloop onderaan de hero — zorgt dat de gele scroll-pijl op zwart leesbaar blijft -->
    <div
        v-if="scrollIndicatorOpacity > 0"
        :style="{
            position: 'fixed',
            left: '0',
            right: '0',
            ...
            height: '240px',
            background:
                'linear-gradient(to top, rgba(12,12,12,0.92) 0%, rgba(12,12,12,0.55) 45%, transparent 100%)',
            zIndex: 2,
            pointerEvents: 'none',
                ...
                <defs>
                    ...
                    >
                        ...
                        <feBlend
                            ...
                            result="effect1_dropShadow_services_scroll"
                        />
                        <feBlend
                            mode="normal"
                            in="SourceGraphic"
                            in2="effect1_dropShadow_services_scroll"
                            result="shape"
                        />
                    </filter>
                </defs>
    ...
    <section id="home" ref="heroRef" class="bg-[#0c0c0c]" style="height: 550vh">
        <div class="sticky top-0 h-screen" :style="heroBgStyle">
            ...
            <div
                ...
                :style="{ ...floatingBaseStyle(0), ...floatingStyle(0) }"
            >
                <HlsVideo
                    v-if="
                        heroFloatingImages?.[0] &&
                        isVideo(heroFloatingImages[0])
                    "
                ...
                />
                <img
                    v-else-if="heroFloatingImages?.[0]"
                    :src="heroFloatingImages[0]!"
                    class="h-full w-full object-cover"
                    alt=""
                />
            ...
            <div
                class="absolute rounded-[20px]"
                :style="{ ...floatingBaseStyle(1), ...floatingStyle(1) }"
            >
                <HlsVideo
                    v-if="
                        heroFloatingImages?.[1] &&
                        isVideo(heroFloatingImages[1])
                    "
                ...
                />
                <img
                    v-else-if="heroFloatingImages?.[1]"
                    :src="heroFloatingImages[1]!"
                    class="h-full w-full object-cover"
                    alt=""
                />
            ...
            <div
                class="absolute rounded-[20px]"
                :style="{ ...floatingBaseStyle(2), ...floatingStyle(2) }"
            >
                <HlsVideo
                    v-if="
                        heroFloatingImages?.[2] &&
                        isVideo(heroFloatingImages[2])
                    "
                ...
                />
                <img
                    v-else-if="heroFloatingImages?.[2]"
                    :src="heroFloatingImages[2]!"
                    class="h-full w-full object-cover"
                    alt=""
                />
            ...
            <div
                class="absolute rounded-[20px]"
                :style="{ ...floatingBaseStyle(3), ...floatingStyle(3) }"
            >
                <HlsVideo
                    v-if="
                        heroFloatingImages?.[3] &&
                        isVideo(heroFloatingImages[3])
                    "
                    :src="heroFloatingImages[3]!"
                    autoplay
                    loop
                    muted
                    playsinline
                ...
                />
                <img
                    v-else-if="heroFloatingImages?.[3]"
                    :src="heroFloatingImages[3]!"
                    class="h-full w-full object-cover"
                    alt=""
                />
    ...
    >
        <div
            :style="casesStickyStyle"
            class="relative h-full overflow-hidden bg-[#0c0c0c]"
        >
            <!-- "↙ Cases" titel rechtsboven — pijl links van de titel -->
            <div
                class="absolute top-6 right-[59px] z-10 flex items-center gap-3"
            >
            ...
            >
                <div
                    ...
                    :style="[
                        casesTrackStyle,
                        { gap: CARD_GAP + 'px', paddingLeft: '59px' },
                    ]"
                >
                    <a
                        ...
                        class="relative shrink-0 cursor-pointer overflow-hidden rounded-[20px] select-none"
                        :style="{
                            width: caseCardSize(i).width + 'px',
                            height: caseCardSize(i).height + 'px',
                            filter:
                                hoveredCaseIndex !== null && !isCaseHovered(i)
                                    ? 'blur(17px)'
                                    : 'none',
                    ...
                    >
                        ...
                        <HlsVideo
                            ...
                            :ref="
                                (comp: any) =>
                                    setCaseVideoRef(comp?.videoEl ?? null, i)
                            "
                            :src="caseItem.video"
                            loop
                            muted
                            ...
                            :class="
                                isCaseHovered(i) ? 'opacity-100' : 'opacity-0'
                            "
                        />
                        ...
                        <div class="absolute inset-0 z-10">
                            ...
                            <div
                                class="absolute right-5 bottom-5 transition-opacity duration-300"
                                :style="{ opacity: isCaseHovered(i) ? 1 : 0 }"
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                ...
                                >
                                    <path
                                        d="M21.3457 53.3944L54.8379 52.672L54.0832 17.6802"
                                        stroke="#FFC700"
                                        stroke-width="5"
                                    />
                                    <path
                                        d="M53.8163 51.6939L1.72841 1.80544"
                                        stroke="#FFC700"
                                        stroke-width="5"
                                    />
                                </svg>
    ...
    >
        <div
            class="relative flex min-h-[50vh] flex-col items-center bg-[#0c0c0c] px-8 pt-[72px] pb-[52px]"
        >
            <!-- Quote + attributie -->
            <div
            ...
            >
                ...
                <div class="flex items-center gap-4">
                    <img
                        v-if="resolvedTestimonialLogo"
                        :src="resolvedTestimonialLogo"
                        alt=""
                        class="h-8 w-auto object-contain brightness-0 invert"
                    />
                    <img
                        v-if="resolvedTestimonialAvatar"
                        :src="resolvedTestimonialAvatar"
                        alt=""
                ...
                <div
                    ...
                    :style="[
                        clientLogosStyle,
                        { height: `${testimonialLogoRowHeight}px` },
                    ]"
                >
                    <div
                    ...
                    >
                        <div
                            ...
                            :style="{
                                width: `${TESTIMONIAL_LOGO_WIDTH}px`,
                                height: `${TESTIMONIAL_LOGO_HEIGHT}px`,
                            }"
                        >
                            <!-- Geel ingekleurd via CSS-mask, ongeacht de bronkleur van het logo -->
                            <div
                                class="h-full w-full"
                                :style="{
            ...
            <div
                class="relative z-10 mt-5 h-8 w-full max-w-[calc(100vw-118px)]"
            >
                <div class="ml-auto h-8 w-[71px]">
                    <div
                        class="pointer-events-none flex h-8 w-[71px] items-center justify-center opacity-90"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="71"
                        ...
                        >
                            <path
                                d="M62.125 19.3467L69.7044 11.7673L61.7856 3.84849"
                                stroke="#FFC700"
                                stroke-width="1.59984"
                            />
                            <path
                                d="M69.2497 11.7649H46.1721"
                                stroke="#FFC700"
                                stroke-width="1.59984"
                            />
                            <g opacity="0.5">
                                <path
                                    d="M8.71094 4.4834L1.13153 12.0628L9.05031 19.9816"
                                    stroke="white"
                                    stroke-width="1.59984"
                                />
                                <path
                                    d="M1.58424 12.0613L24.6618 12.0613"
                                    stroke="white"
                                    stroke-width="1.59984"
                                />
                            </g>
    ...
    <section
        ...
        ref="teamRef"
        style="
            height: max(115vh, calc(1000px + 15vh));
            position: relative;
            z-index: 10;
            background: #ffc700;
        "
    >
        <div class="sticky top-0 min-h-screen bg-[#ffc700]">
            <div
                class="relative flex min-h-screen w-full flex-col overflow-visible pb-[80px]"
            >
                <!-- "Team Lemon" achtergrondtekst — exits upward faster than photos -->
                <div
                    class="relative z-10 flex min-h-[720px] w-full items-center justify-center pt-[140px]"
                >
                    <div
                ...
                <!-- Beschrijving + knop -->
                <div
                    class="relative z-20 mt-auto flex w-full justify-center px-8"
                >
                    <div
